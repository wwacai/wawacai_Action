...
[rewrite_local]
https://fhnc-ii-game.lw0591.com/ws\Sparam=\S+UserID\S+TokenID\S+MsgID\S+Type\S+Diamonds\S+ url script-request-header https://raw.githubusercontent.com/wwacai/wawacai_Action/main/Task/wwcqmdff.js

hostname = *.lw0591.com

[task_local]
1 * * * * https://raw.githubusercontent.com/wwacai/wawacai_Action/main/Task/wwcqmdff.js, tag=柠檬全民大丰收, img-url=http://nm66.top/1.jpg, enabled=true
...
const qmdffheaderArr = [],qmdffurlArr = []
...
if ($.isNode()) {
  if (process.env.qmdffheader && process.env.qmdffheader.indexOf('#') > -1) {
  qmdffheader = process.env.qmdffheader.split('#');
  console.log(`您选择的是用"#"隔开\n`)
  }
  else if (process.env.qmdffheader && process.env.qmdffheader.indexOf('\n') > -1) {
   qmdffheader = process.env.qmdffheader.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   qmdffheader = process.env.qmdffheader.split()
  };
  Object.keys(qmdffheader).forEach((item) => {
        if (qmdffheader[item]) {
          qmdffheaderArr.push(qmdffheader[item])
        }
    });

  if (process.env.qmdffurl && process.env.qmdffurl.indexOf('#') > -1) {
   qmdffurl = process.env.qmdffurl.split('#');
   console.log(`您选择的是用"#"隔开\n`)
  }
  else if (process.env.qmdffurl && process.env.qmdffurl.indexOf('\n') > -1) {
   qmdffurl = process.env.qmdffurl.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   qmdffurl = process.env.qmdffurl.split()
  };
  Object.keys(qmdffurl).forEach((item) => {
        if (qmdffurl[item]) {
          qmdffurlArr.push(qmdffurl[item])
        }
    });
...
if($request&&$request.url.indexOf("UserID")>=0) {
    ...
    if(qmdffurl){
       $.setdata(qmdffurl,`qmdffurl${status}`)
       $.log(`[${qmdff}] 获取: 成功: ${qmdffurl}`)
    }
    if(qmdffheader){
      $.setdata(qmdffheader,`qmdffheader${status}`)
...
async function hb1(){
 return new Promise((resolve) => {
    ...
    let plant6_url = {
     ...
     headers: JSON.parse(qmdffheader),
    }
