...
[rewrite_local]
https://qmlzc-api.lw0591.com/v1/userinfo\S+ url script-request-header https://raw.githubusercontent.com/wwacai/wawacai_Action/main/Task/wwcqmlzc.js

hostname = qmlzc-api.lw0591.com

[task_local]
1 * * * * https://raw.githubusercontent.com/wwacai/wawacai_Action/main/Task/wwcqmlzc.js, tag=柠檬全民来找茬, img-url=circles.hexagongrid.fill.system, enabled=true
...
if ($.isNode()) {
  if (process.env.xnmheader && process.env.xnmheader.indexOf('#') > -1) {
  xnmheader = process.env.xnmheader.split('#');
  console.log(`您选择的是用"#"隔开\n`)
  }
  else if (process.env.xnmheader && process.env.xnmheader.indexOf('\n') > -1) {
   xnmheader = process.env.xnmheader.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   xnmheader = process.env.xnmheader.split()
  };
  Object.keys(xnmheader).forEach((item) => {
        if (xnmheader[item]) {
          xnmheaderArr.push(xnmheader[item])
        }
    });

  if (process.env.xnmurl && process.env.xnmurl.indexOf('#') > -1) {
   xnmurl = process.env.xnmurl.split('#');
   console.log(`您选择的是用"#"隔开\n`)
  }
  else if (process.env.xnmurl && process.env.xnmurl.indexOf('\n') > -1) {
   xnmurl = process.env.xnmurl.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   xnmurl = process.env.xnmurl.split()
  };
  Object.keys(xnmurl).forEach((item) => {
        if (xnmurl[item]) {
          xnmurlArr.push(xnmurl[item])
        }
    });
...
async function cqg(){
 return new Promise((resolve) => {
    let fj = xnmurl.match(/api_token=(\S+)"/)[1]
    let plant6_url = {
