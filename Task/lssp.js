...
let status, videoid,myid,supportvideoid,supportrank,show,message,note,random,wkpower,spanswer,spbdid,gold,lsstate,lssign
...
//getmoney1
async function getmoney1(){
 return new Promise((resolve) => {
    let lsstate = $.getdata('lsstate')
    let lssign = $.getdata('lssign')
    let getmoney1_url = {
        url: `https://wx.fxzhe.top/app/index.php?i=118&t=0&m=jyt_txvideo&v=1.64&from=wxapp&c=entry&a=wxapp&do=getmoney&state=${lsstate}&${lssign}&id=7`,
        headers: {
'Accept-Encoding' : `gzip, deflate, br`,
'Connection' : `keep-alive`,
'Referer' : `https://servicewechat.com/wxd43f74ad84dba818/2/page-frame.html`,
...
'Accept-Language' : `zh-cn`
   ...
   $.get(getmoney1_url,async(error, response, data) =>{
    try{
        ...
        message += '🔔【乐上视频-个人信息】 '
        if(result.errno == 0){
          console.log(`🎈乐上视频-提现10元成功。\n`)
          message += `🎈乐上视频-提现10元成功。\n`
          }else{
          console.log('👀个人信息错误'+result.message+"\n")
          message += '👀个人信息错误'+result.message+"\n"
          }
...
async function getmoney2(){
 return new Promise((resolve) => {
    ...
    let lssign = $.getdata('lssign')
    let getmoney2_url = {
        url: `https://wx.fxzhe.top/app/index.php?i=118&t=0&m=jyt_txvideo&v=1.64&from=wxapp&c=entry&a=wxapp&do=getmoney&state=${lsstate}&${lssign}&id=6`,
        headers: {
'Accept-Encoding' : `gzip, deflate, br`,
'Connection' : `keep-alive`,
'Referer' : `https://servicewechat.com/wxd43f74ad84dba818/2/page-frame.html`,
...
'Accept-Language' : `zh-cn`
   ...
   $.get(getmoney2_url,async(error, response, data) =>{
    try{
        ...
        message += '🔔【乐上视频-个人信息】 '
        if(result.errno == 0){
          console.log(`🎈乐上视频-提现50元成功。\n`)
          message += `🎈乐上视频-提现50元成功。\n`
          }else{
          console.log('👀个人信息错误'+result.message+"\n")
          message += '👀个人信息错误'+result.message+"\n"
          }
   ...
   $.get(readvideo_url,async(error, response, data) =>{
    try{
        ...
        if(result.errno == 0){
          ...
          console.log('👀乐上视频错误'+result.message+"\n")
          message += '👀乐上视频错误'+result.message+"\n"
          }
        }catch(error) {
          $.logErr(error, response);
      } finally {
