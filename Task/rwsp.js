...
async function userinfo(){
 return new Promise((resolve) => {
	...
	"Referer": "https://servicewechat.com/wx138ed41d6f5cee3a/12/page-frame.html",
	"Content-Type": "application/x-www-form-urlencoded",
	"Host": "zm.shujumagician.com",
	"User-Agent": "Mozilla/5.0(iPhone;CPUiPhoneOS13_6_1likeMacOSX)AppleWebKit/605.1.15(KHTML,likeGecko)Mobile/15E148MicroMessenger/8.0.1(0x1800012a)NetType/4GLanguage/zh_CN",
   ...
   $.get(readvideo_url,async(error, response, data) =>{
    try{
        ...
        if(result.errno == 0){
          ...
          console.log('👀热舞视频错误'+result.message+"\n")
          message += '👀热舞视频错误'+result.message+"\n"
          }
        }catch(error) {
          $.logErr(error, response);
      } finally {
   ...
   $.get(withdraw_url,async(error, response, data) =>{
    try{
        ...
        if(result.errno == 0){
          ...
          console.log(`热舞视频-成功提现0.3元\n`)
          message += `热舞视频-成功提现0.3元\n`
        }else{
